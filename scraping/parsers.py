		...
		if not not_found_jobs:
			...
			if main_div:
				...
				for div_1 in div_list:
					...
					div_company_4 = div_company_3.find('div', attrs={'class': 'vacancy-serp-item__meta-info-company'})
					company = div_company_4.text
					jobs.append({'title': title.text, 'href': href, 'description': description, 'company': company})
			else:
		...
		if not not_found_jobs:
			...
			if main_div:
				...
				for div_1 in div_list:
					...
					jobs.append(
						{'title': title.text, 'href': domain + href, 'description': description, 'company': company})
			else:
				errors.append({'url': url, 'error': 'div не найден'})
		else:
			errors.append({'url': url, 'error': 'Вакансий по данному запросу нет'})
